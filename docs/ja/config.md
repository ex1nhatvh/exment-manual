...
#### 所属ユーザ設定可制御機能
- 設定キー : EXMENT_SHOW_AUTH_SETTING
- 既定値 : false
- 役割 : trueにすることと、組織の標準フォームへ[「所属ユーザ設定可能」項目追加](/ja/organization?id=所属ユーザ設定可能項目追加)することで、組織登録・修正画面で、対象組織に所属するユーザのユーザ設定を表示・非表示の制御が出来るようになります。falseの場合は所属するユーザのユーザ設定が常に表示されます。
### 並び順設定
