...
ユーザーがブラウザで認証し、アプリの利用を承認することで、認証され、APIを使用できるようになる形式です。Web上からAPIを実行する場合におすすめです。

1. **Password Grant Token(パスワード形式)**  
認証リクエストに、ログインID・パスワード文字列を含めることで、APIを使用できるようになる形式です。Web画面を呼び出しにくい、クライアントアプリにおすすめです。

1. **API Key(APIキー形式)**  
APIアプリの作成時に、APIキーを作成します。その後、認証リクエストに、APIキーを含めることで、認証され、 APIを使用できるようになる形式です。バッチ実行におすすめです。


### 1. OAuth 2.0, Authorization Code Flow(画面ログイン形式)
この認証方式は、新たに構築するWebサービス（例：会計システム）から、ユーザーがExmentのログインを行い、APIを使用できるようになる形式です。  
...
- 必要事項を入力します。  
	...
	**リダイレクトURL**  
	ユーザーが画面から認証を完了した時に、リダイレクトするURLを記入してください。  
...
この認証方式は、認証リクエストに、ログインID・パスワード文字列を含めることで、APIを使用できるようになる形式です。

- ユーザーが画面でID・パスワードを入力する必要がないため、バッチ処理などでも実行可能です。  

- ユーザーのIDパスワードを、認証リクエストに含める必要があります。  
...
### 3. API Key(APIキー形式)
APIアプリの作成時に、APIキーを作成します。その後、認証リクエストに、APIキーを含めることで、認証され、 APIを使用できるようになる形式です。
...
## エラーコード一覧(確認中)
API実行時に、何らかの理由でエラーが発生した場合、エラーコードが返却されます。  
それらのエラーコードと、HTTPレスポンスのステータスコードの一覧について、まとめています。  

| Exmentエラーコード | HTTPステータスコード | 説明 | 
| ---- | ---- | ---- |
| 101 | 403 | そのエンドポイント・カスタムテーブルにアクセスするための権限がありません。実行するユーザーの権限を見直してください。 |
| 102 | 400 | 実行時のクエリ文字列や、送信された値に不備があります。値を見直してください。 |
| 103 | 400 | 主にデータの追加・更新の実施時に、送信された値のバリデーション(検証)で不備が発生しています。送信値を見直してください。 |
| 105 | (確認中) | 削除できないデータです。 |
| 106 | 403 | API認証時のスコープに誤りがあります。スコープの値を見直してください。 |
| 107 | 400 | idを指定して、特定のデータを取得する場合、その値が存在していません。idを指定してください。 |
| 108 | 401 | API認証時のアクセストークンに誤りがあるか、有効期限が切れています。アクセストークンを見直してください。 |
| 109 | 400 | データ新規作成時もしくは削除時に、同時に実施できる件数(標準：100件)を超過したリクエストを実施した場合に発生します。 |
| 111 | 500 | プラグイン(API)の実施時に、指定したプラグインが存在しない場合に発生します。プラグイン名を見直してください。 |
| 112 | 400 | ビューを対象にしたAPIで、テーブルとビューが異なるものだった場合に発生します。 |
| 113 | 400 | ビューを対象にしたAPIで、サポートされていないビュー種類を指定していた場合に発生します。 |
| 201 | (確認中) | ワークフローのステータスにより、ロックされているデータに対し、データの更新・削除などを実施した場合に発生します。 |
| 202 | 400 | 「現在のワークフローの実行値」APIを実行時、ワークフローをスタートしていなかった場合に発生します。 |
| 203 | 400 | 「特定のデータに対し、作業ユーザーの一覧を取得」「特定のデータに対し、実行できる次のアクションの一覧を取得」APIを実行時、ワークフローが完了していた場合に発生します。 |
| 204 | 400 | 「特定のデータに対し、ワークフローを実行」APIを実行時、指定のアクションIDが、実行可能でなかった場合に発生します。 |
| 301 | 400 | IP制限により、APIが実行できません。 |
