...
- 列種類が選択肢／選択肢（値・見出し）／選択肢（他のテーブルから）の場合は、複数選択の途中切り替えにより検索不可が発生します。
...
### 検索バーによる検索（フリーワード検索）
フリーワード検索は、現在選択しているカスタムビューから独立した機能になります。  
検索範囲は対象テーブルの全データで、すべてのフリーワード検索列を対象に検索を行います。  
そのため結果は現在選択しているカスタムビューではなく、全件ビューの形式で表示されます。
...
> 現在、Google検索のような、半角スペースを入力しての複数列のAND検索には対応しておりません。
