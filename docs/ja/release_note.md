...
1. 機能改修
    - パフォーマンス向上のための修正（Workflow, View関連）詳細は[こちら](/ja/additional_queue)


## v6.1.1 (2024/07/24)

1. 不具合修正
    - LDAPでPHP8.2に対応できていなかった不具合の修正
    - 集計ビューに関する不具合の修正
...
1. 機能追加
    - 二次元バーコード機能の追加　詳細は[こちら](/ja/2d_barcode)
1. 不具合修正
    - SSO設定後に「サイトロゴ」と「SSOログインボタンのアイコン」が表示されない不具合の修正


## v6.0.4 (2024/06/05)

1. 不具合修正
    - アップグレード後、パスワードが変更できなくなった不具合の修正 (laravel-admin)
    - 集計ビューの合計の計算に「削除済みデータ」が含まれている不具合の修正
    - ビューの編集画面に表示順が表示される場合、表示列選択の上下移動ボタンの上に表示列の表示順入力欄が表示されてしまう不具合の修正
